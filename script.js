...
for(let i = 2026 ; i >=2005 ; i--){
    let pushVari = ` <button onclick="buttonToSearch(this)" class="px-4 py-1 bg-blue-600 text-white font-semibold uppercase rounded cursor-pointer " >
                   ${i}
                 </button>`
    arr.push(pushVari)
}
...
function buttonToSearch(e){
      card_div.innerHTML = " "
    // IMG_POSTER , TITLE , RANK,AKA , ACTORS , YEAR,


    fetch(apiUrl+e.innerText).then(res=>res.json()).then((data)=>{
     const movieDetails =    data.description  
